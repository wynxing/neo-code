...
TUI v2 **不提供** wide/focus/compact 等多布局模式。所有终端尺寸使用同一套 Focus-Only 布局：
...
- **< 100 列**：Soft Inspector 自动隐藏。Stream 占满宽度。Ambient Status 省略非关键字段。
- **≥ 100 列**：Soft Inspector 自动出现在右侧（约 28–35 列，无边框，弱色文字）。
...
| Soft Inspector | 大屏右侧（≥ 100 列自动显示） | 无边框右列，弱色文字 | 展示 Runtime、Tools、Files 摘要；宽度不足时自动隐藏 |
...
*本规范是 TUI v2 文档集的子文档。架构总览见 [架构导航](./tui-v2-architecture-hub.md)，数据契约见 [数据/契约](./tui-v2-data-and-gateway.md)，工程实现见 [工程实现](./tui-v2-implementation-guide.md)。*
