...
func TestProviderEstimateGenerateAndThinkingErrors(t *testing.T) {
	...
	drained := drainMimoEvents(events)
	if len(drained) != 3 {
		t.Fatalf("expected 3 events, got %d", len(drained))
	}
	if !strings.Contains(string(requestBody), `"thinking":{"type":"disabled"}`) {
		t.Fatalf("request body missing disabled thinking: %s", string(requestBody))
	}
...
func drainMimoEvents(events <-chan providertypes.StreamEvent) []providertypes.StreamEvent {
