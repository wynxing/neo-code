...
func TestProviderEstimateGenerateAndThinkingErrors(t *testing.T) {
	...
	req := providertypes.GenerateRequest{
		...
		ThinkingConfig: &providertypes.ThinkingConfig{Enabled: true},
	}
	...
	drained := drainQwenEvents(events)
	if len(drained) != 3 {
		t.Fatalf("expected 3 events, got %d", len(drained))
	}
	if !strings.Contains(string(requestBody), `"enable_thinking":true`) ||
		!strings.Contains(string(requestBody), `"temperature":0.6`) {
		t.Fatalf("request body missing qwen thinking params: %s", string(requestBody))
	}
...
func drainQwenEvents(events <-chan providertypes.StreamEvent) []providertypes.StreamEvent {
